...
Host 半侧提供 `/dsh-supervisor/*` 本地路由：读取 GitHub Release manifest、选择当前平台制品、下载到 `$DSH_HOME/downloads/desktop-supervisor/`、校验 SHA-256、只打开已校验的文件或目录、读取 `$DSH_HOME/supervisor/control.json`，并用 descriptor token 转发本机 supervisor pairing 和 restart 请求，按启动错误文本诊断候选插件 id，并转发针对某个失败插件的定点禁用请求。开发 profile 可配置 `localArtifactPath` 指向本地构建出的安装包；远端 manifest 不可用时，页面会把该安装包作为 `local-dev` 制品展示并原地校验。Client 半侧把入口注册到“设置 → 桌面启动”。桌面托盘端尚未安装时，该页面仍支持读取 manifest、选择本机平台安装包、下载或本地校验、打开已校验下载文件和刷新状态；restart 和定点启动恢复需要托盘 App 运行后才可用。
