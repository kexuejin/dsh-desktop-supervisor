...
Host 半侧提供 `/dsh-supervisor/*` 本地路由：读取 GitHub Release manifest、选择当前平台制品、下载到 `$DSH_HOME/downloads/desktop-supervisor/`、校验 SHA-256、只打开已校验的文件或目录、读取 `$DSH_HOME/supervisor/control.json`，并用 descriptor token 转发本机 supervisor pairing。Client 半侧把 P0 入口注册到“设置 → 桌面启动”。桌面托盘端尚未安装时，该页面仍支持读取 manifest、选择本机平台安装包、下载、校验、打开已校验下载文件和刷新状态；restart、Safe Mode、启动失败恢复和本机进程日志需要托盘 App 运行后才可用。
