...
The tray menu includes “Check for Updates” and “Install Update and Relaunch”. The Web companion proxies the same operations as `/dsh-supervisor/check-update` and `/dsh-supervisor/install-update`, so Settings → Desktop launch can operate the tray updater without exposing the token to browser code.
